...
func startCacheServer(self string, addrs []string, gee *discache.Group) {
	peers := discache.NewHTTPPool(self)
	peers.Set(addrs...)
	gee.RegisterPeers(peers)
	log.Println("discache is running at", self)
...
func main() {
	var port int
	var api bool
	flag.IntVar(&port, "port", 8001, "discache server port")
